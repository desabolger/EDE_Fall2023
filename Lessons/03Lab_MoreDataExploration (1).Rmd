...
```{r DataExercise-1a}
vectorH <- c(81,83,84,84,78,71,79,82,85,84) 
vectorH

vectorL <- c(55,56,58,60,61,63,63,64,64,63) 
vectorL
...
```{r DataExercise-1b}
FtoC <- function(vectorLetter)
{
  vectorC = (vectorLetter - 32)/1.8

return(vectorC)
vectorC
}
CvectorH <- FtoC(vectorH)
CvectorH


CvectorL <- FtoC(vectorL)
CvectorL
```
...
```{r DataExercise-1c}
temp <- data.frame(vectorL, vectorH, CvectorL, CvectorH) #
names(temp) <- c("F Low","F High","C Low", "C High"); View(temp) # 
```
...
```{r DataExercise-1d}
summary(temp)
sd(temp$`F Low`) # only works with one line of numbers
```
