...
df_withdrawals <- data.frame("Month" = Month,
                             "Year" = rep(2022,12), #is this right
                             "PWSID" = PWSID, 
...
df_withdrawals <- df_withdrawals[order(as.Date(df_withdrawals$Date, format="%Y/%m/%d")),]
...
ggplot(df_withdrawals, aes(x=Date, y=Vector)) + 
  ...
  labs(title = paste("2022 Max daily withdrawals", Water_System_Name),
       subtitle = Ownership, #confused on order
       y="Withdrawal (mgd)", #fix unit
       x="Date") 
...
Water_System_Name <- the_website %>% html_nodes(Water_System_Name) %>% html_text()
...
plotting <- ggplot(df_withdrawals, aes(x=Date, y=Vector)) + 
  geom_point() +
  geom_path(group =1) +
  labs(title = paste(the_year,"Max daily withdrawals", Water_System_Name),
       subtitle = Ownership, #confused on order
       y="Withdrawal (mgd)", #fix unit
       x="Date") 
...
ggplot() + 
  geom_point(NewSet, mapping = aes(x=Date, y=Vector), color = 'blue') +
  geom_path(NewSet, mapping = aes(x=Date, y=Vector, group = 1), color = "blue") +
  geom_point(Asheville, mapping = aes(x=Date, y=Vector), color = 'green') +
  geom_path(Asheville, mapping = aes(x=Date, y=Vector, group = 1), color = "green") +
  labs(title = paste(the_year,"Max daily withdrawals from ", Asheville$Water_System_Name, " and ", NewSet$Water_System_Name),
       subtitle = paste(Asheville$Water_System_Name, " & ", NewSet$Water_System_Name), #confused on order
       y="Withdrawal (mgd)", #fix unit
       x="Date") 
...
> Answer:
> It looks like it is increasing over time. 
