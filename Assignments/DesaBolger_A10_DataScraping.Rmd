...
Month <- the_website %>% html_nodes(Month) %>% html_text()
...
#Plot 
ggplot(NewSet, aes(x=Date, y=Vector)) + 
  geom_point() +
  geom_line() +
  geom_smooth(method="loess",se=FALSE) +
...
NewSet <-scrape.it("03-32-010", 2015) 
view(NewSet)
Asheville <-scrape.it("01-11-010", 2015)  #is this combining the data


ggplot() + 
  geom_point(NewSet, mapping = aes(x=Date, y=Vector), color = 'blue') +
  geom_line(NewSet, mapping = aes(x=Date, y=Vector), color = "blue") +
  geom_point(Asheville, mapping = aes(x=Date, y=Vector), color = 'green') +
  geom_line(Asheville, mapping = aes(x=Date, y=Vector), color = "green") +
