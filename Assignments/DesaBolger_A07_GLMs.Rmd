...
> Answer: (do i need to put anything here)???
...
#5 is this right or reversed?? do i need to invert?

ChemGraph <- ggplot(ChemPhysics.processed, aes(x = temperature_C, y = depth)) +
  geom_point(alpha = 0.75, size = 1.5) +
  scale_y_reverse(limits=c(15, 0))+
  stat_smooth(method = lm, se = F, color = 'black')+
  xlim(0,35)+
  xlab(expression(paste("Temperature (C)" )))+
  ylab(expression(paste("Depth")))
#prob add a title
...
> Answer: It suggests that (may need to reverse) as temperature decreases, so does depth.  The distribution seems to fit the linear regression line, suggesting it may be linear. 
...
#is this the right test and the right order. 
...
#I think its daynum and depth, maybe year 4 based on p value.??


#10 PAUSED HERE.. having issues...
