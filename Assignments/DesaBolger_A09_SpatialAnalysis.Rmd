...
GageHeight <- st_read(here('Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
  filter(STATEFP == 31)
  view(GageHeight)
...
GageHeight$geometry[1]
#NAD83. 


#5. Plot the data
ggplot(data=GageHeight) + geom_sf()
...
ggplot() + 
  geom_sf(data=GageHeight,aes(fill = E_POV),alpha=0.3) + 
  scale_fill_gradient2(low="red",high="blue",midpoint = 60000) + 
  geom_sf(data=GageLocation)
